...
if ( ! class_exists( 'FormHandler' ) ) {
	...
	class FormHandler {
		 ...
		 * @var string
		 */
		protected $form_option = 'mowc_customer_validation_settings';
		...
		protected function __construct() {
			...
			add_filter( 'mowc_phone_dropdown_selector', array( $this, 'get_phone_number_selector' ), 1, 1 );

			if ( SessionUtils::is_otp_initialized( $this->form_session_var )
			|| SessionUtils::is_otp_initialized( $this->form_session_var2 ) ) {

				add_action( 'mowc_otp_verification_successful', array( $this, 'handle_post_verification' ), 1, 7 );

				add_action( 'mowc_otp_verification_failed', array( $this, 'handle_failed_verification' ), 1, 4 );

				add_action( 'mowc_unset_session_variable', array( $this, 'unset_otp_session_variables' ), 1, 0 );
			}

			add_filter( 'mowc_is_ajax_form', array( $this, 'is_ajax_form_in_play' ), 1, 1 );

			add_filter( 'mowc_is_login_or_social_form', array( $this, 'is_login_or_social_form' ), 1, 1 );
		...
		public function validate_challenge( $otp_type, $req_var = 'mo_otp_token', $otp_token = null ) {
			do_action( 'mowc_validate_otp', $otp_type, $req_var, $otp_token );
		}
		...
		public function basic_validation_check( $message ) {
			if ( $this->is_form_enabled() && MoUtility::is_blank( $this->otp_type ) ) {
				do_action( 'mowc_registration_show_message', MoMessages::showMessage( $message ), MoConstants::ERROR );
				return false;
			}
			return true;
		}
		...
		/**Function to get the button texts.
		 */
		public function get_button_text() {
			return mowc_( $this->button_text ); }

		/**Function to get the form details.
		 */
		public function get_form_details() {
			return $this->form_details; }

		/**Function to restrict the duplicate users.
		 */
		public function restrict_duplicates() {
			return $this->restrict_duplicates; }

		/**Function to bypass user login.
		 */
		public function bypass_for_logged_in_users() {
			return $this->by_pass_login; }

		/**Function to check if form is a login or socual form.
		 */
		...
		/**Function: To check if form is an Ajax form.
		 */
		public function is_ajax_form() {
			return $this->is_ajax_form; }

		/**Function: To check if form is an addon form.
		 */
		public function is_add_on_form() {
			return $this->is_add_on_form; }

		/**Function: Get form documents.
		 */
